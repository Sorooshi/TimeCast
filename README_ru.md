# Пакет для Прогнозирования Временных Рядов
...
- **4 Режима Обучения**: Комплексный рабочий процесс для различных случаев использования
...
Пакет предоставляет комплексный CLI с 4 различными режимами:
...
|-------|----------|-------------------|
| `tune` | Настройка гиперпараметров + обучение с лучшими параметрами | Первый раз с новыми данными/моделью |
| `apply` | Обучение с ранее настроенными параметрами (или по умолчанию) | Использование существующих настроенных параметров |
| `apply_not_tuned` | Обучение только с параметрами по умолчанию | Базовое сравнение или быстрое тестирование |
| `report` | Отображение сохраненных результатов предыдущих запусков | Анализ и сравнение |
...
- `--mode`: Режим обучения (по умолчанию: apply)
