...
|------|-----|---------------|------------------|
| `tune` | فقط بهینه‌سازی فراپارامترها | تقسیم train/val از داده‌های اصلی | پارامترهای تنظیم‌شده |
| `train` | آموزش با پارامترهای تنظیم‌شده (`--train_tuned true`) یا پیش‌فرض (`--train_tuned false`) | تقسیم train/val از داده‌های اصلی | **نمودارها، تاریخچه، معیارها، پیش‌بینی‌ها** |
| `predict` | بارگیری مدل آموزش‌دیده و پیش‌بینی (`--predict_tuned true/false`) | **نیاز به فایل داده تست جداگانه** | نتایج پیش‌بینی |
| `report` | نمایش تحلیل جامع آزمایش‌ها | - | خلاصه‌های تحلیل |

**🎨 جدید در حالت آموزش**: به طور خودکار نمودارهای آموزش/اعتبارسنجی (اتلاف، R²، MAPE) و تاریخچه کامل آموزش را ذخیره می‌کند!
