...
|------|-----|---------------------|
| `tune` | فقط بهینه‌سازی فراپارامترها | اولین بار با داده/مدل جدید |
| `train` | آموزش با پارامترهای تنظیم‌شده (`--train_tuned true`) یا پیش‌فرض (`--train_tuned false`) | حالت آموزش اصلی |
| `predict` | بارگیری مدل آموزش‌دیده و پیش‌بینی (`--predict_tuned true/false`) | ایجاد پیش‌بینی |
| `report` | نمایش تحلیل جامع آزمایش‌ها | تحلیل و مقایسه |
